...
def HasReachableReduction (C : MatchingCutColoring G) : Prop :=
  ∃ M : MatchingCut G, C.toMatchingCut.FlipReachable M ∧
    (ContainsPositiveTailReducer M.toColoring ∨
     ContainsCutEnhancer M.toColoring)

theorem HasReachableReduction.of_current_ptr (C : MatchingCutColoring G)
...
theorem HasReachableReduction.of_current_ce (C : MatchingCutColoring G)
...
theorem HasReachableReduction.after_flip (C : MatchingCutColoring G)
    {M₁ : MatchingCut G} {r s : V}
    (hflip : C.toMatchingCut.IsFlipAt M₁ r s)
    (hresult : HasReachableReduction M₁.toColoring) :
    HasReachableReduction C := by
...
same. -/
theorem HasReachableReduction.of_swapSides (C : MatchingCutColoring G)
