import Subcubic.Lemma4_12.Case3Shared
...
/-- The chord analysis at the start of the inlined Lemma 4.11 argument.
...
theorem path6_induced_or_reduction
    ...
    HasReachableReduction C ∨ FormsInducedPath6 G a b c d e f := by
  classical
  ...
  have current (hout : ContainsPositiveTailReducer C ∨ ContainsCutEnhancer C) :
    ...
    hout.elim (HasReachableReduction.of_current_ptr C)
      (HasReachableReduction.of_current_ce C)
  by_cases hac : G.Adj a c
  · exact Or.inl (current (lemma4_2 C a b c d hab ha hb hcd hc hd (by
  ...
  · exact Or.inl (current (lemma4_2 C e f c d hef he hf hcd hc hd (by
...
theorem path6_subgraph_with_clean_ends
  ...
  rcases path6_induced_or_reduction C hpath ha hb hc hd he hf with
    hresult | hinduced
  · exact hresult
...
theorem lemma4_11_inline
  ...
  have noOutcome (hout : ContainsPositiveTailReducer C ∨ ContainsCutEnhancer C) :
      False :=
  ...
  · have extendRight {a b c d e f : V}
      ...
      have hxc : x ≠ c := by
        ...
        subst x
        apply noOutcome
        apply lemma4_2 C e f c d hef he hf hcd hc hd
      ...
      have hex : ¬ G.Adj e x := by
        intro hex
        apply noOutcome
        apply lemma4_2 C e f x y hef he hf hxy hx hy
      ...
      have hya : ¬ G.Adj y a := by
        ...
        exact noOutcome (lemma4_7 C hcycle ha hb hc hd he hf hx hy)
      ...
      · by_cases hNoBlueA : ∀ z, G.Adj a z → C.color z ≠ .blue
        · exact noResult (lemma4_8 C hp8 ha hb hc hd he hf hx hy
            hNoBlueA hNoRedY)
      ...
      · push Not at hNoRedY
        ...
        have hsub10 : FormsPath10Subgraph G a b c d e f x y z w := by
          ...
          · intro i j hij
            ...
            fin_cases i <;> fin_cases j <;> simp [graphOfEdges] at hij
            all_goals assumption
        exact noResult (lemma4_6 C hsub10 ha hb hc hd he hf hx hy hz hw)
    ...
    · have hNoBlueF : ∀ x, G.Adj f x → C.color x ≠ .blue := by
      ...
      · have hNoBlueA : ∀ x, G.Adj a x → C.color x ≠ .blue := by
        ...
        exact noResult (lemma4_10 C hinduced ha hb hc hd he hf
          hNoBlueA hNoBlueF)
