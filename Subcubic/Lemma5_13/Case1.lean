import Subcubic.Lemma5_13.ThirdNeighborSetup
import Subcubic.Lemma5_2

/-! Case (1) of Lemma 5.13: the two exposed third neighbors are adjacent. -/

namespace Subcubic

variable {V : Type*} [Fintype V] {G : SimpleGraph V}

theorem lemma5_13_case_ef
    (C : GoodColoring G) {a b c d : V}
    (hpath : FormsInducedPath4 G a b c d)
    (ha : C.color a = .red) (hb : C.color b = .red)
    (hc : C.color c = .blue) (hd : C.color d = .blue)
    (Q : Lemma5_13ThirdNeighborConfiguration C a b c d)
    (hef : G.Adj Q.e Q.f) : HasReachableNegativeReduction C := by
  classical
  by_contra hresult
  ...
  · obtain ⟨M, hflip⟩ := hflip
    ...
    have hout := lemma5_2 D c Q.f b Q.e Q.hcf hcD hfD Q.hbe hbD heD (by
      simp [fourVertexCrossEdgeCount, hbc.symm, hfe]
      omega)
    have hD := hout.elim (HasReachableNegativeReduction.of_current_ntr D)
      (HasReachableNegativeReduction.of_current_ce D)
    exact hresult (HasReachableNegativeReduction.after_flip C hflip hD)
