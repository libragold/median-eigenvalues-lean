import Subcubic.Lemma5_9.Case1
import Subcubic.Lemma5_4

/-! Lemma 5.9, Case (2): `i` is adjacent to neither `g` nor `h`. -/
...
theorem lemma5_9_case_i_not_adj_g_h
  ...
  have d_has_no_fourth {z : V}
    ...
    C.not_adj_fourth_neighbor (Or.inr hd) hcd.symm hde hdi
      ...
      (by intro q; subst i; simp_all)
      (by intro q; subst i; simp_all) hzc hze hzi
  have g_other : ∀ z, G.Adj g z → z ≠ h → D.color z = .reddish := by
    intro z hgz hzh
    by_cases hzf : z = f
    · simpa [hzf] using hfD
    have hzSide := C.other_neighbor_of_blue_is_redSide hg hh hgh hgz hzh
    have hze : z ≠ e := by
      intro q; subst z
    ...
    · apply reddish_of_untouched_reddish C hflip hz
      · intro hzd
        ...
        · intro q; subst z; simp_all
        · intro q; subst z; exact hig hgz.symm
      · exact hze
      · intro q; subst z; simp_all
    · exact (noCurrentCE hce).elim
  have h_other : ∀ z, G.Adj h z → z ≠ g → D.color z = .reddish := by
    intro z hhz hzg
    have hzSide := C.other_neighbor_of_blue_is_redSide hh hg hgh.symm hhz hzg
    have hz : C.color z = .reddish := by
      rcases hzSide with hz | hz
      · exact (hNoRedAtH z hhz hz).elim
      · exact hz
    have hze : z ≠ e := by intro q; subst z; simp_all
    apply reddish_of_untouched_reddish C hflip hz
    · intro hzd
      apply d_has_no_fourth _ hze _ hzd.symm
      · intro q; subst z; simp_all
      · intro q; subst z; exact hih hhz.symm
    · exact hze
    · intro q; subst z; simp_all
  have hdone := lemma5_4 D.swapSides
    (by simp [hgD]) (by simp [hhD]) hgh
    (by
      intro z hgz hzh
      have hz := g_other z hgz hzh
      simp [hz])
    (by
      intro z hhz hzg
      have hz := h_other z hhz hzg
      simp [hz])
  have hdone' := HasReachableNegativeReduction.of_swapSides D hdone
  exact hresult (HasReachableNegativeReduction.after_flip C hflip hdone')
